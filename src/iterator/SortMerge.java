 ...
 */
public class SortMerge implements IStream, GlobalConst {
    ...
    public SortMerge(AttrType in1[],
        ...
        if (!in1_sorted) {
            try {
                BigTableMain.orderType = 4;
                p_i1 = new MapSort(in1, s1_sizes, am1, join_col_in1,
                        order, amt_of_mem / 2, sortFld1Len);
            } catch (Exception e) {
        ...
        if (!in2_sorted) {
            try {
                BigTableMain.orderType = 4;
                p_i2 = new MapSort(in2, s2_sizes, am2, join_col_in2,
                        order, amt_of_mem / 2, sortFld2Len);
            } catch (Exception e) {
                throw new SortException(e, "Sort failed");
            }
        }
        ...
        try {
            TempMap1.setMapHeader(_in1, s1_sizes);
            map1.setMapHeader(_in1, s1_sizes);
            TempMap2.setMapHeader(_in2, s2_sizes);
            map2.setMapHeader(_in2, s2_sizes);
        } catch (Exception e) {
    ...
    public Map getNext()
        ...
        while (true) {
            if (process_next_block) {
                ...
                comp_res = MapUtils.CompareMapWithMap(map1, map2, jc_in1-1);
                ...
                while (MapUtils.CompareMapWithMap(map1, TempMap1, jc_in1-1) == 0) {
                ...
                while (MapUtils.CompareMapWithMap(map2, TempMap2, jc_in2-1) == 0) {
