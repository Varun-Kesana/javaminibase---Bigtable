...
public class Projection {
    ...
    public static void Join(Map t1, AttrType[] type1,
                            ...
                            Map joinedMap, FldSpec[] perm_mat,
                            int nOutFlds
    )
        ...
        for (int i = 0; i < nOutFlds; i++) {
            // Concat row names.
            if (i == 0) {
                joinedMap.setStrFld(i + 1, t1.getStrFld(perm_mat[i].offset) + ":" + t2.getStrFld(perm_mat[i].offset));
                continue;
            }
            // Keep greater of two timestamps.
            if (i == 2) {
                if (t1.getTimeStamp() > t2.getTimeStamp()) {
                    joinedMap.setIntFld(i + 1, t1.getIntFld(perm_mat[i].offset));
                } else {
                    joinedMap.setIntFld(i + 1, t2.getIntFld(perm_mat[i].offset));
                }
                continue;
            }
                ...
                case RelSpec.outer:        // Field of outer (t1)
                    switch (type1[perm_mat[i].offset - 1].attrType) {
                        ...
                        case AttrType.attrReal:
                            joinedMap.setFloFld(i + 1, t1.getFloFld(perm_mat[i].offset));
                            break;
                        case AttrType.attrString:
                            joinedMap.setStrFld(i + 1, t1.getStrFld(perm_mat[i].offset));
                            break;
                        default:
                ...
                case RelSpec.innerRel:        // Field of inner (t2)
                    switch (type2[perm_mat[i].offset - 1].attrType) {
                        case AttrType.attrInteger:
                            ...
                            break;
                        case AttrType.attrReal:
                            joinedMap.setFloFld(i + 1, t2.getFloFld(perm_mat[i].offset));
                            break;
                        case AttrType.attrString:
                            joinedMap.setStrFld(i + 1, t2.getStrFld(perm_mat[i].offset));
                            break;
                        default:
